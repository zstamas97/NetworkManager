                       ...
                       const NML3ConfigData *l3cd)
{
    ...
    if (!IS_IPv4 && l3cd) {
        /* FIXME: nm_dhcp_utils_merge_new_dhcp6_lease() relies on "life_starts"
         *   option, but internal client  never sets that. So for the internal
         *   client this has no effect. */
...
bytearray_variant_to_string(NMDhcpClient *self, GVariant *value, const char *key)
{
    ...
    /* Since the DHCP options come through environment variables. Let's only
     * accept ASCII.
     */
